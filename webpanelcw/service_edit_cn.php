...
if (isset($_GET['service_id'])) {
    ...
    $stmt->execute();
    $row_service_cn = $stmt->fetch(PDO::FETCH_ASSOC);
...
if (isset($_POST['del-img'])) {
    ...
    if ($delete_img) {
        echo "<meta http-equiv='refresh' content='0;url=service_edit_cn?service_id=$service_id'>";
    }
}
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img_cover['size'] > 0 && $img_cover['error'] == 0) {
            if (move_uploaded_file($img_cover['tmp_name'], $filePath1)) {
                $service = $conn->prepare("UPDATE service_cn SET service_name = :service_name ,content = :content, 
        ...
        $service = $conn->prepare("UPDATE service_cn SET service_name = :service_name ,content = :content WHERE service_id = :service_id");
        $service->bindParam(":service_name", $service_name);
        $service->bindParam(":content", $text_content);
        $service->bindParam(":service_id", $service_id);
        $service->execute();
                    ...
                    <div class="card">
                            ...
                            <div class="title-img">
                                ...
                                <div id="gallery-cover">
                                    <div class='box-edit-img-cover'>
                                        ...
                                        <img class='edit-img-cover' id='previewImg-cover' src="upload/upload_service/<?php echo $row_service_cn['cover_img'] ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="product-name">
                                <h6>Service Name</h6>
                                <input type="text" value="<?php echo $row_service_cn['service_name'] ?>" name="service_name" class="form-control">
                            </div>

                            <div class="content-text">
                                <h6>Content</h6>
                                <textarea name="text-content"><?php echo $row_service_cn['content'] ?></textarea>
                                <script>
                                    tinymce.init({
                                        selector: 'textarea',
                                        height: "350",
