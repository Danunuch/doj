    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img1['size'] > 0 && $img1['error'] == 0) {
            if (move_uploaded_file($img1['tmp_name'], $filePath1)) {
                $edit_about_us = $conn->prepare("UPDATE about_us_cn SET content = :content, img = :img WHERE id = :id");
        ...
        $edit_about_us = $conn->prepare("UPDATE about_us_cn SET content = :content WHERE id = :id");
        $edit_about_us->bindParam(":content", $content);
        $edit_about_us->bindParam(":id", $id);
        $edit_about_us->execute();
        if ($edit_about_us) {
