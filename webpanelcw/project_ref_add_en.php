            ...
            if (in_array($fileActExt1, $allow)) {
                if ($cover_img['size'] > 0 && $cover_img['error'] == 0) {
                    if (move_uploaded_file($cover_img['tmp_name'], $filePath1)) {
                        $project = $conn->prepare("INSERT INTO project_en (project_name, customer, location, project_start, project_finish, product_list ,status , cover_img, ref_id)
                ...
                $project = $conn->prepare("INSERT INTO project_en (project_name, customer, location, project_start, project_finish, product_list ,status , ref_id)
...
$select_stmt = $conn->prepare("SELECT * FROM category_ref_en");
$select_stmt->execute();
