...
$data_typepro = $conn->prepare("SELECT * FROM category_product");
$data_typepro->execute();
$row_type_product = $data_typepro->fetchAll();

$data_sub = $conn->prepare("SELECT * FROM category_sub");
$data_sub->execute();
...
if (isset($_POST['edit-type-pro'])) {
    ...
    $edit_product_ref = $conn->prepare("UPDATE category_product SET type_name = :type_name WHERE pt_id = :pt_id");
    ...
    if ($edit_product_ref) {
        ...
        echo "<meta http-equiv='refresh' content='2;url=type_product'>";
    } else {
        echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
...
if (isset($_POST['edit-sub'])) {
    ...
    $edit_sub = $conn->prepare("UPDATE category_sub SET sub_name = :sub_name WHERE id = :id");
...
if (isset($_POST['delete_ref'])) {
    ...
    $q_sub = $conn->prepare("SELECT * FROM category_sub WHERE pt_id = :id");
    $q_sub->bindParam(":id", $cat_id);
    $q_sub->execute();
    $row_sub_cat = $q_sub->fetchAll();

    //delete main
    $del_cat_main = $conn->prepare("DELETE FROM category_product WHERE pt_id = :id");
    $del_cat_main->bindParam(":id", $cat_id);
    $del_cat_main->execute();

    //delete sub
    for ($i = 0; $i < count($row_sub_cat); $i++) {
        $del_in_sub = $conn->prepare("DELETE FROM category_sub WHERE pt_id = :pt_id");
...
if (isset($_POST['delete_sub'])) {
    ...
    $del_sub = $conn->prepare("DELETE FROM category_sub WHERE id = :id");
            ...
            <section class="section">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="flex-end">
                            <a href="type_product_en"><button type="button" class="btn btn-edit1">EN</button></a>
                            <a href="type_product_cn"><button type="button" class="btn btn-edit1">CN</button></a>
                            <a href="type_add"><button type="button" class="btn btn-edit1">Add</button></a>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="card-header">
                            ...
                            <div class="flex-end">
                                <a href="sub_add"><button type="button" class="btn btn-edit1">Add</button></a>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table">
