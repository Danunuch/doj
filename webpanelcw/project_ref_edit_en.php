...
if (isset($_GET['project_id'])) {
    ...
    $project_data = $conn->prepare("SELECT * FROM project_en WHERE project_id = :project_id");
    $project_data->bindParam(":project_id", $project_id);
    $project_data->execute();
    $row_project_en = $project_data->fetch(PDO::FETCH_ASSOC);
...
if (isset($_POST['del-img'])) {
    ...
    if ($delete_img) {
        echo "<meta http-equiv='refresh' content='0;url=project_ref_edit_en?project_id=$project_id'>";
    }
}
...
if (isset($_POST['edit-project'])) {
    ...
    $project = $conn->prepare("UPDATE project_en SET project_name = :project_name, customer = :customer, location = :location, project_start = :project_start,
                        ...
                        <div class="card-body">
                            ...
                            <div class="content">
                                <div class="project-name">
                                    <h6>Project Name</h6>
                                    <input type="text" name="project_name" value="<?php echo $row_project_en['project_name'] ?>" class="form-control">
                                    <h6>Customer</h6>
                                    <input type="text" name="customer" value="<?php echo $row_project_en['customer'] ?>" class="form-control">
                                    <h6>Location</h6>
                                    <input type="text" name="location" value="<?php echo $row_project_en['location'] ?>" class="form-control">
                                    <h6>Project Start</h6>
                                    <input type="text" name="project_start" value="<?php echo $row_project_en['project_start'] ?>" class="form-control">
                                    <h6>Project Finish</h6>
                                    <input type="text" name="project_finish" value="<?php echo $row_project_en['project_finish'] ?>" class="form-control">
                                    <h6>Product List</h6>
                                    <input type="text" name="product_list" value="<?php echo $row_project_en['product_list'] ?>" class="form-control">
                                </div>
