...
$data_news = $conn->prepare("SELECT * FROM news_cn ORDER BY news_id DESC");
$data_news->execute();
$row_news_cn = $data_news->fetchAll();

if (isset($_POST['change-status'])) {
    $check = $_POST['check'];
    $news_id = $_POST['news_id'];

    $stmt = $conn->prepare("UPDATE news_cn SET status = :status WHERE news_id =  :news_id");
...
if (isset($_POST['delete_all'])) {
    if (count((array)$_POST['ids']) > 0) {
        ...
        $del_news = $conn->prepare("DELETE FROM news_cn WHERE news_id in ($all)");
        $del_news->execute();
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <?php
                                    for ($i = 0; $i < count($row_news_cn); $i++) {
                                        $p_id = $row_news_cn[$i]["news_id"];
                                    ...
                                    ?>
                                        <tbody>
                                            <tr>
                                                <td align="center">
                                                    <input type="checkbox" class="form-check-input checkbox checkbox-select" name="ids[]" value=<?php echo $row_news_cn[$i]['news_id'] ?>>
                                                </td>
                                                <td align="center" width="20%">
                                                    <img width="50%" src="upload/upload_news/<?php echo $row_news_cn[$i]['cover_img']; ?>" alt="">
                                                </td>
                                                <td align="center"><?php echo $row_news_cn[$i]['news_name']; ?></td>
                                                <td align="center"><?php echo $row_news_cn[$i]['created_at']; ?></td>
                                                <td align="center">
                                                    <a type="input" class="btn" <?php if ($row_news_cn[$i]['status'] == "on") {
                                                                                    echo " style='background-color: #06c258'";
                                                                                } else {
                                                                                    echo " style='background-color: #ff4122'";
                                                                                } ?> data-bs-toggle="modal" href="#status<?php echo $row_news_cn[$i]['news_id'] ?>" id="setting"><i class="bi bi-gear"></i></a>

                                                </td>
                                                <td align="center">
                                                    <div class="manage">
                                                        <a href="new_blog_edit_cn?news_id=<?php echo $row_news_cn[$i]['news_id']; ?>"><button type="button" class="btn" style="background-color:#ffc107; color: #FFFFFF;"><i class="bi bi-pencil-square"></i></button></a>
                                        ...
                                        <div class="modal fade" id="status<?php echo $row_news_cn[$i]['news_id'] ?>" data-bs-backdrop="static" aria-hidden="true">
                                                        ...
                                                        <div class="form-check form-switch">
                                                            <form method="post">
                                                                <div class="switch-box">
                                                                    <span>OFF</span>
                                                                    <input type="hidden" name="news_id" value="<?php echo $row_news_cn[$i]['news_id']; ?>">
                                                                    <input class="form-check-input" id="switch-check" name="check" type="checkbox" <?php if ($row_news_cn[$i]['status'] == "on") {
                                                                                                                                                        echo "checked";
                                                                                                                                                    } else {
                                                                                                                                                        echo "";
                                                                                                                                                    } ?>>
