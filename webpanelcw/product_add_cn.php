            ...
            if (in_array($fileActExt1, $allow)) {
                if ($img_cover['size'] > 0 && $img_cover['error'] == 0) {
                    if (move_uploaded_file($img_cover['tmp_name'], $filePath1)) {
                        $product = $conn->prepare("INSERT INTO product_cn(product_name, intro, content, conclude, link_video, cover_img ,status, pt_id, id)
...
$select_stmt = $conn->prepare("SELECT * FROM category_sub_cn");
$select_stmt->execute();
...
<body>
    <div id="app">
        ...
        <div id="main">
            ...
            $stmt_sub = $conn->prepare("SELECT* FROM category_sub_cn");
            $stmt_sub->execute();
