...
$data_project = $conn->prepare("SELECT * FROM project_cn ORDER BY project_id DESC");
$data_project->execute();
$row_project_cn = $data_project->fetchAll();
...
if (isset($_POST['change-status'])) {
    ...
    $stmt = $conn->prepare("UPDATE project_cn SET status = :status WHERE project_id =  :project_id");
...
if (isset($_POST['delete_all'])) {
    if (count((array)$_POST['ids']) > 0) {
        ...
        $del_project = $conn->prepare("DELETE FROM project_cn WHERE project_id in ($all)");
        $del_project->execute();
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <?php
                                    for ($i = 0; $i < count($row_project_cn); $i++) {
                                        $p_id = $row_project_cn[$i]["project_id"];
                                    ...
                                    ?>
                                        <tbody>
                                            <tr>
                                                ...
                                                <td align="center">
                                                    <a type="input" class="btn" <?php if ($row_project_cn[$i]['status'] == "on") {
                                                                                    echo " style='background-color: #06c258'";
                                                                                } else {
                                                                                    echo " style='background-color: #ff4122'";
                                                                                } ?> data-bs-toggle="modal" href="#status<?php echo $row_project_cn[$i]['project_id'] ?>" id="setting"><i class="bi bi-gear"></i></a>
                                        ...
                                        <div class="modal fade" id="status<?php echo $row_project_cn[$i]['project_id'] ?>" data-bs-backdrop="static" aria-hidden="true">
                                                        ...
                                                        <div class="form-check form-switch">
                                                            <form method="post">
                                                                <div class="switch-box">
                                                                    <span>OFF</span>
                                                                    <input type="hidden" name="project_id" value="<?php echo $row_project_cn[$i]['project_id']; ?>">
                                                                    <input class="form-check-input" id="switch-check" name="check" type="checkbox" <?php if ($row_project_cn[$i]['status'] == "on") {
                                                                                                                                                        echo "checked";
                                                                                                                                                    } else {
                                                                                                                                                        echo "";
                                                                                                                                                    } ?>>
