...
$content = $conn->prepare("SELECT * FROM content_cn");
$content->execute();
...
if (isset($_GET['content_id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM content_cn WHERE content_id = :content_id");
...
if (isset($_POST['edit-content'])) {
    ...
    $text = $conn->prepare("UPDATE content_cn SET content1 = :content1, content2 = :content2");
    ...
    if ($text) {
        ...
        echo "<meta http-equiv='refresh' content='1.5;url=content_cn'>";
    } else {
        echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
